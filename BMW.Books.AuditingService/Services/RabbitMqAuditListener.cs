...
{
    public class RabbitMqAuditListener : IAuditListener
    {
        private readonly string _host;
        private readonly string _queue;
        private readonly string _user;
        private readonly string _pass;
        private readonly ILogger<RabbitMqAuditListener> _log;

        private IConnection? _conn;
        private IChannel? _ch;

        public RabbitMqAuditListener(IConfiguration config, ILogger<RabbitMqAuditListener> log)
        {
                ...
                try
                {
                    ...
                    consumer.ReceivedAsync += async (_, ea) =>
                    {
                        try
                        {
                            ...
                            _log.LogInformation("[AUDIT][RabbitMQ] {Message}", message);
