...
namespace BMW.Books.OrderService.Services
{
    public class RabbitMqAuditService : IAuditService
    {
        ...
        private readonly ILogger<RabbitMqAuditService> _logger;

        public RabbitMqAuditService(IConfiguration config, ILogger<RabbitMqAuditService> logger)
        {
        ...
        {
            _logger.LogInformation("Sending audit message to RabbitMQ: {Message}", message);
            _logger.LogInformation("RabbitMQ Host: {Host}, Queue: {Queue}", _host, _queue);
            ...
            var body = Encoding.UTF8.GetBytes($"[OrderService] {DateTime.UtcNow:o} {message}");
